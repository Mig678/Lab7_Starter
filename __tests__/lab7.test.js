describe('Basic user flow for Website', () => {
  ...
  it('Make sure <product-item> elements are populated', async () => {
    ...
    for (let i = 0; i < prodItemsData.length; i++) {
      let currProduct = prodItemsData[i];
    
      if (currProduct.title.length == 0) {
        allArePopulated = false;
      }
    
      if (currProduct.price.length == 0) {
        allArePopulated = false;
      }
    
      if (currProduct.image.length == 0) {
        allArePopulated = false;
      }
    }
  ...
  it('Clicking the "Add to Cart" button should change button text', async () => {
    ...
    // Query thwe button inside the shadow
  ...
  it('Checking number of items in cart on screen', async () => {
     ...
     * Remember to remove the .skip from this it once you are finished writing this test.
     */
    // click every button that still says Add to Cart
    ...
    // get cart count
    const cartNum = await page.$eval('#cart-count', (cart) => cart.innerText);
  ...
  it('Checking number of items in cart on screen after reload', async () => {
     ...
     * Remember to remove the .skip from this it once you are finished writing this test.
     */
    // reload page second truy
    await page.reload();

    // check every button says Remove from Cart
    const allRemove = await page.$$eval('product-item', (products) => {
  ...
  it('Checking the localStorage to make sure cart is correct', async () => {
    ...
    // should have all 20 product ids
